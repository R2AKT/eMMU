...
; -----------------------------------------------------------------------------
; =============================================================================
; ПРОЦЕДУРА: hw_i8251_init
; Вход:  DE = Физический адрес инициализируемого 160-байтового TTY-слота
; Задача: Инициализировать ВИ53 и ВВ51А, прописать векторы HAL в хвост слота
...
; hw_i8251_init:
    ...
    ; ; --- ЭТАП 1: НАСТРОЙКА КАНАЛОВ ТАЙМЕРА КР580ВИ53 ---
    ; ; Канал 0: Тактирование RxC (приемник), Режим 3 (Меандр), LSB/MSB
    ; MVI  A, PIT_CW_CH0_M3             ; 036H = SC=00, RL=11, M=011, BCD=0
    ; OUT  UART_PIT_CMD
    ; MVI  A, 00CH                    ; Делитель Rx = 1_843_200 / (9600 × 16) = 12
    ; OUT  UART_PIT_CH0
    ; MVI  A, 000H                    ; Старший байт делителя = 0
    ; OUT  UART_PIT_CH0
    
    ; ; Канал 1: Тактирование TxC (передатчик), Режим 3 (Меандр), LSB/MSB
    ; MVI  A, PIT_CW_CH1_M3             ; 076H = SC=01, RL=11, M=011, BCD=0
    ; OUT  UART_PIT_CMD
    ; MVI  A, 00CH                    ; Делитель Tx = 1_843_200 / (9600 × 16) = 12
    ; OUT  UART_PIT_CH1
    ; MVI  A, 000H                    ; Старший байт делителя = 0
    ; OUT  UART_PIT_CH1

    ; ; --- ЭТАП 2: ИНИЦИАЛИЗАЦИЯ БИС USART КР580ВВ51А ---
    ; ; Тройной выстрел нуля для гарантированного сброса автомата состояний БИС
    ; XRA  A
    ; OUT  UART_CMD_PORT
    ; OUT  UART_CMD_PORT
    ; OUT  UART_CMD_PORT
    
    ; ; Внутренний программный сброс (Internal Reset)
    ; MVI  A, I8251_CMD_RESET         ; 040H
    ; OUT  UART_CMD_PORT
    ; NOP
    
    ; ; Инструкция режима: 8 бит, без чётности, 1 стоп-бит, фактор ×16
    ; MVI  A, I8251_MODE_8N1_X16      ; 04EH — подтверждено на реальном железе
    ; OUT  UART_CMD_PORT
    ; NOP
    
    ; ; Инструкция команды: Tx/Rx разрешены, RTS/DTR активны, сброс ошибок
    ; MVI  A, I8251_CMD_START         ; 027H
    ; OUT  UART_CMD_PORT

    ; ; --- ЭТАП 3: ПРОШИВКА HAL ВЕКТОРОВ В ХВОСТ ТЕКУЩЕГО TTY СЛОТА ---
    ; XCHG                            ; HL = База TTY-слота, DE = мусор

    ; ; Записываем адрес hw_i8251_put_char в O_TTY_HW_PUT_CHAR (+149)
    ; PUSH H
    ...
    ; ; Записываем адрес hw_i8251_get_char в O_TTY_HW_GET_CHAR (+151)
    ; PUSH H
    ...
    ; ; Записываем адрес hw_i8251_poll_status в O_TTY_HW_POLL_STAT (+153)
    ; PUSH H
    ...
    ; ; --- ЭТАП 4: ФИКСАЦИЯ ПАРАМЕТРОВ ЧИПА В СТРУКТУРЕ ---
    ; PUSH H
    ; LXI  D, O_TTY_HW_BASE_PORT
    ; DAD  D                          ; HL -> O_TTY_HW_BASE_PORT (+155)
    ; MVI  M, UART_DATA_PORT          ; Базовый порт данных (0A0H)
    
    ; INX  H                          ; HL -> O_TTY_HW_DIV_LOW (+156)
    ; MVI  M, 0CH                     ; Делитель частоты (12)
    ; INX  H                          ; HL -> O_TTY_HW_DIV_HIGH (+157)
    ; MVI  M, 00H                     ; Старший байт делителя
    
    ; INX  H                          ; HL -> O_TTY_HW_CHIP_TYPE (+158)
    ; MVI  M, CHIP_TYPE_I8251         ; 01H = КР580ВВ51А
    ; POP  H
